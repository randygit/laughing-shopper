...
exports.users = function(req,res) {
    ...
    User.find({verified:true},{name:1, email:1, role:1, deactivated:1}, {sort: {name:1}}, function(err,Users) {
        if(!err) {
            res.json(Users);
        }
        else {
...
exports.edit = function (req, res) {
    //should save userId of who updated this record, audit trail
    ...
    if (id) {
        User.findById(id, function (err, User) {
            if(!err) {
                if(User) {
                    console.log('User.findById ' + JSON.stringify(User)); 
